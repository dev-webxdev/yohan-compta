...
<div class="settings-history">@foreach($periods as $period)<article><strong>À partir du {{ $period->effective_from->format('d/m/Y') }}</strong><span>Brut {{ Money::formatCents($period->hourly_gross_rate_cents) }}/h · net {{ Money::formatCents($period->hourly_net_rate_cents) }}/h · seuil {{ Time::formatDuration($period->weekly_threshold_minutes) }} · panier {{ Money::formatCents($period->meal_allowance_cents) }} dès {{ Time::formatClock($period->meal_allowance_time_minutes) }}</span></article>@endforeach</div>
</section>
</div>
@endsection
