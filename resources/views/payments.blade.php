...
<section class="panel spacer-top payment-history-panel">
    ...
    @forelse($payments as $payment)
        <article class="payment-row">
            ...
            <div class="payment-amount">{{ Money::formatCents($payment->amount_cents) }}<small>Heures sup payées : @if($paymentHours[$payment->id]['minutes'] === null)non affectées pour le moment @else{{ $paymentHours[$payment->id]['indicative'] ? '≈ ' : '' }}{{ Time::formatDuration($paymentHours[$payment->id]['minutes']) }}@endif</small>@if(($documentCounts[$payment->id]??0)>0)<a class="associated-documents-link" href="{{ route('library.index',['target'=>'payment:'.$payment->id]) }}"><i class="fa-solid fa-paperclip"></i> Documents associés : {{ $documentCounts[$payment->id] }}</a>@endif</div>
